...
// Episodes renders
// Episode cards render to the page
// Arrange Act Assert
test('episodes component renders to page', ()=>{
...
test('episodes in seasons render in component', ()=>{
    const { queryByTestId, queryAllByTestId, rerender } = render(<Episodes episodes={[]} />);
    expect(queryAllByTestId(/episode-div/i)).toHaveLength(0)

    rerender(<Episodes episodes={mockEpisodeData} />);
    expect(queryAllByTestId(/episode-div/i)).toHaveLength(8);
